...
class HotelManagementSystem:
    def __init__(self,root):
        ...
        self.root.geometry("1550x800+0+0") #  width x height + window start from x= + window start from y=
        ...
        cust_btn.grid(row=0,column=0,pady=1) #-->grid() used to place the button in frame
        ...
        room_btn.grid(row=1,column=0,pady=1) #-->pady to keep buttons separate
        
        details_btn=Button(btn_frame,text="DETAILS",width=22,command=self.detailsroom,font=("times new roman",14,"bold"),bg="black",fg="gold",bd=0,relief=RIDGE,cursor="hand1")
        ...
        logout_btn=Button(btn_frame,text="LOGOUT",width=22,command=self.logout,font=("times new roman",14,"bold"),bg="black",fg="gold",bd=0,relief=RIDGE,cursor="hand1")
        ...
        lblimg4.place(x=0,y=190,width=230,height=210)

        img5 = Image.open("D:/STUDY MATERIAL/SEM4/RPPOOP/Hotel_Management_System/images/downImg2.jpg")
        img5 = img5.resize((230,240),Image.LANCZOS)
        self.photoimg5 = ImageTk.PhotoImage(img5)
        lblimg5 = Label(main_frame, image = self.photoimg5, bd = 4,relief = RIDGE ) # bd--> border,relief = border style
        lblimg5.place(x=0,y=380,width=230,height=250)


    def cust_details(self):
        self.new_window = Toplevel(self.root) #-->Toplevel creates a new window on top of root
        self.app = Cust_Win(self.new_window) #-->creates instance of Cust_win
